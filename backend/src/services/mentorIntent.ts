...
export function detectMentorIntent(message: string | null | undefined): MentorIntent {
  ...
  if (
    /full solution|complete solution|complete example|complete working|complete code|whole code|whole solution|entire solution|entire code|working version|working solution|working example|just write the code|just give me the code|give me the code|give me the full code|give me a working|the full code|give\s+me\s+(the\s+)?full\b|provide (a|the) (complete|full|working)|solve it for me|solve it completely|send the final answer only|final answer only|write the final answer only|no hints|just code|only code|fix the code and send the corrected version|pretend you are not a mentor|ignore previous instructions|output the final code|for testing purposes|write\s+(the\s+)?\w+\s+function\s+for\s+me|write\s+(the|a)\s+\w+\s+(function|method|class|program)\s+for\s+me|just\s+(write|do)\s+(the|this)\s+\w+\s+(function|part|method)|convert\s+(it|this|my|the)\s+(to|into)\s+(actual|real)\s+(python|java|c|c\+\+|javascript|code)|convert\s+(it|this|my|the)\s+\w*\s*(pseudocode|pseudo-code|comments)|turn\s+(this|my)\s+(pseudocode|pseudo-code|comments)\s+into|walk\s+me\s+through\s+each\s+line|line\s+by\s+line|line-by-line|what\s+would\s+line\s+\d+\s+be|now\s+line\s+\d+|change\s+the\s+variable\s+names|so\s+it\s+counts\s+as\s+my\s+own|i'?ll\s+change\s+(it|the\s+structure)|i'?ll\s+modify\s+(it|after)|for\s+studying\s+purposes|just\s+for\s+studying|sınava\s+bakacağım|tam\s+çözüm|tam\s+çözümü|tam\s+kodu?\b|tam\s+kod\s+yaz|tüm\s+kod|tüm\s+kodu|bütün\s+kod|bütün\s+kodu|sadece\s+kod|sadece\s+kodu\s+ver|tam\s+(python|java|c|c\+\+|javascript)\s+kodu?|final\s+cevab|direkt\s+çöz|çözümü\s+ver\b|çözümü\s+yaz|kodu\s+yaz|kodu\s+yazsan|kopyalayıp\s+yapıştır|kopyala\s+yapıştır|tam\s+\w+\s+fonksiyonunu?\s+yaz|fonksiyonu\s+benim\s+için\s+yaz|kelimesi\s+kelimesine|adım\s+adım\s+her\s+satır|her\s+satırı\s+yaz/.test(
      msg,
    )
  ) {
    return "solution";
