...
export function detectMentorIntent(message: string | null | undefined): MentorIntent {
  ...
  if (
    /full solution|complete solution|complete example|complete working|complete code|whole code|whole solution|entire solution|entire code|working version|working solution|working example|just write the code|just give me the code|give me the code|give me the full code|give me a working|the full code|give\s+me\s+(the\s+)?full\b|provide (a|the) (complete|full|working)|solve it for me|solve it completely|send the final answer only|final answer only|write the final answer only|no hints|just code|only code|fix the code and send the corrected version|pretend you are not a mentor|ignore previous instructions|output the final code|for testing purposes|write\s+(the\s+)?\w+\s+function\s+for\s+me|write\s+(the|a)\s+\w+\s+(function|method|class|program)\s+for\s+me|just\s+(write|do)\s+(the|this)\s+\w+\s+(function|part|method)|fill\s+in\s+(the\s+)?(missing\s+)?(parts?|blanks?|implementation|body)|fill\s+(it\s+)?in\s+for\s+me|complete\s+(the\s+)?(missing|empty)\s+(parts?|function|body)|you\s+(just\s+)?write\s+(the\s+)?implementation|just\s+(write|do|give\s+me|show\s+me)\s+(the\s+|a\s+|that\s+)?(loop|for-?loop|while-?loop|function|method|body|implementation|block|section|chunk|snippet|part|piece|fragment|for|while|if-?block|else-?block|switch|inner|core)|(?:can\s+you\s+)?(?:just\s+)?write\s+(the\s+|a\s+|just\s+the\s+|me\s+the\s+)?(loop|for-?loop|while-?loop|block|body|section|chunk|snippet|inner|core|word\s+counting|counting)\s+(for\s+me|please|part)|write\s+(the|a|me\s+the|just\s+the)\s+\w+\s+(loop|for-?loop|while-?loop|block|section|chunk|body|snippet)\s+for\s+me|i'?ll\s+(write|do|handle|plug|integrate)\s+(the\s+)?(rest|everything\s+else|main|the\s+other\s+parts?)|i'?ll\s+plug\s+it\s+in|i\s+just\s+need\s+to\s+see\s+it|just\s+(this\s+one\s+time|this\s+one\s+part|this\s+piece)|(?:maybe\s+|like\s+|just\s+|only\s+)?\d{1,2}\s*-?\s*(\d{1,2}\s*)?lines?\s+(tops?|max|maximum|or\s+so)?|show\s+me\s+what\s+(the|a)\s+correct\s+(version|solution|implementation)\s+looks\s+like|what\s+(the|a)\s+correct\s+(version|solution|implementation)\s+looks\s+like|compare\s+with\s+the\s+correct\s+version|demonstrate\s+(by|your)\s+writing|must\s+demonstrate\s+by\s+writing|demonstrate\s+(your\s+)?capabilities\s+by\s+writ|prove\s+(it|yourself)\s+by\s+writ|paste\s+the\s+(corrected|working|complete)\s+version|send\s+me\s+the\s+(corrected|working|complete)\s+version|rewrite\s+it\s+correctly|the\s+whole\s+working\s+version|whole\s+working\s+(code|program|solution)|convert\s+(it|this|my|the)\s+(to|into)\s+(actual|real|the\s+real)\s+(python|java|c|c\+\+|javascript|code)|convert\s+(it|this|my|the)\s+\w*\s*(pseudocode|pseudo-code|comments)|translate\s+(it|this|my|the)\s+(to|into)\s+(python|java|c|c\+\+|javascript)|turn\s+(this|my)\s+(pseudocode|pseudo-code|comments)\s+into|walk\s+me\s+through\s+each\s+line|line\s+by\s+line|line-by-line|what\s+would\s+line\s+\d+\s+be|now\s+line\s+\d+|change\s+the\s+variable\s+names|so\s+it\s+counts\s+as\s+my\s+own|count(s)?\s+as\s+my\s+own\s+work|i'?ll\s+change\s+(it|the\s+structure)|i'?ll\s+modify\s+(it|after)|for\s+studying\s+purposes|just\s+for\s+studying|sınava\s+bakacağım|tam\s+çözüm|tam\s+çözümü|tam\s+kodu?\b|tam\s+kod\s+yaz|tüm\s+kod|tüm\s+kodu|bütün\s+kod|bütün\s+kodu|sadece\s+kod|sadece\s+kodu\s+ver|tam\s+(python|java|c|c\+\+|javascript)\s+kodu?|final\s+cevab|direkt\s+çöz|çözümü\s+ver\b|çözümü\s+yaz|kodu\s+yaz|kodu\s+yazsan|kopyalayıp\s+yapıştır|kopyala\s+yapıştır|tam\s+\w+\s+fonksiyonunu?\s+yaz|fonksiyonu\s+benim\s+için\s+yaz|sadece\s+\w+\s*\(\s*\)\s+fonksiyonunu\s+yaz|kelimesi\s+kelimesine|adım\s+adım\s+her\s+satır|her\s+satırı\s+yaz/.test(
      msg,
    )
  ) {
    return "solution";
